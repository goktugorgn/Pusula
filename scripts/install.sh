...
# -----------------------------------------------------------------------------
# Step 4b: Configure Unbound (Setup Remote Control)
# -----------------------------------------------------------------------------
configure_unbound() {
    log_info "Configuring Unbound..."
    
    # 1. Setup Control Certificates
    ...
    if [[ -d "/etc/unbound/unbound.conf.d" ]]; then
        cat > "/etc/unbound/unbound.conf.d/99-pusula.conf" << EOF
# Pusula Configuration
# Auto-generated by install.sh

server:
    # Ensure local control interface is enabled
    # Note: control-enable is handled in remote-control section
...
include: "/etc/unbound/pusula-managed.conf"
EOF
        log_success "Created /etc/unbound/unbound.conf.d/99-pusula.conf"
        
        # Verify and restart
        if unbound-checkconf &>/dev/null; then
            systemctl restart unbound || true
            log_success "Unbound restarted"
        else
            log_warn "Unbound config check failed. Please verify /etc/unbound/unbound.conf manually."
        fi
    else
        log_warn "/etc/unbound/unbound.conf.d not found. Manual Unbound configuration required."
    fi
}
